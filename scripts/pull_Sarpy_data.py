...
def get_update_date(date_query_url, time_field):
    ...
    epoch_time = features[0][u'attributes'][time_field] # the epoch time of the last time the data was updated in ms
...
def write_data(file_path, date, zips, cases):
    ...
    try:
        ...
        if quoted_date in dates:
            overwrite = True
            print("WARNING: data has already been updated today... overwriting data for today with recently fetched data.")
    ...
    if (len(dates) > 1):
        num_dates = len(dates) - 2 # List of dates begins with an empty string.
    ...
    for zip_code in new_data:
        if len(new_data[zip_code]) != expected_entries:
            ...
            sys.exit("ERROR: Inconsistency in number of data points for zip code %s. Data failed to update." % zip_code)
